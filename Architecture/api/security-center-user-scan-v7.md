...
IPC. Все четыре режима панели и точечная проверка из Nautilus завершаются
системным уведомлением с итоговыми счётчиками.

Приватное состояние runtime может физически находиться внутри home, но trusted
Module Manager передаёт его как защищённое исключение. Full scan не заходит в
этот subtree, а file/folder scan такого пути отклоняется с `protected_path`.
Это предотвращает сканирование изменяемой SQLite-базы и объектов карантина.
