...
- materialize поддерживает только копирование snapshot: план и approval одноразовые,
  имеют TTL, источники привязаны по identity/metadata/SHA-256, а частичный результат
  удаляется при ошибке; перемещение пока не реализовано;
