...
class CapabilityRegistryTests(RegistryTestCase):
    ...
    def test_enabled_modules_publish_complete_capability_contracts(self) -> None:
        ...
        security_profile = by_id["security.scan.run"]
        self.assertEqual(
            set(security_profile.input_schema["required"]),
            {"resource_id", "mode"},
        )
        self.assertEqual(
        ...
        security_findings = by_id["security.findings.list"]
        self.assertEqual(
            set(security_findings.input_schema["properties"]),
            {"state", "limit"},
        )
