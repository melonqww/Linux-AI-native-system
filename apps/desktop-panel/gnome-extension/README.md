...
Desktop Portal: абсолютный путь вводить не нужно. Все проверки продолжаются в
фоне, показывают число обработанных файлов и байтов, время и угрозы; текущую
проверку можно отменить. После завершения GNOME показывает итоговое уведомление,
а последние запуски остаются в короткой локальной истории. Автоматический quick
scan раз в 24 часа включается отдельным переключателем и по умолчанию выключен.
