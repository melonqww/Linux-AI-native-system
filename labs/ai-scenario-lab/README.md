...
История развития матрицы, всех существенных Foundation-прогонов, обнаруженных
проблем и подтверждённых улучшений ведётся отдельно в
[`LAB-HISTORY.md`](LAB-HISTORY.md). Сырые отчёты остаются локальными и не
подменяются этой очищенной сводкой.

Проверяемые агрегаты и результаты отдельных случаев опубликованы в
[`public-evidence/`](public-evidence/README.md). Они генерируются строгим
allowlist-экспортом: диалоги, model prose, traces, локальные пути и данные хоста
туда не попадают.

Последний завершённый полный Foundation-прогон `20260925T081344.764208Z`:
**113/113 passed** на seeds 7/19. Это статус backend-кандидата для охваченной
матрицы, а не утверждение о готовности GNOME/Linux beta; ограничения перечислены
в [истории](LAB-HISTORY.md) и отчёте.
...
Failed checks, model-event status, compiler diagnostics и step `error_code`
преобразуются в evidence до классификации. Model prose, traceback и реальные
пути не анализируются и не попадают в fingerprint; effect-check сохраняет только
тип эффекта. Для `UNKNOWN` отчёт показывает безопасные rule/evidence вместо
одного имени case.
...
- production `files.operations` с тем же prepare/approval/commit flow для
  inspect, create-directory, move, rename и recoverable trash;
...
Capability router → Permission Gateway → Orchestrator
    ↓
index/search/copy/file-operations только внутри VirtualComputer
    ↓
...
8. Проверяются сообщения, capability, количество найденных/затронутых файлов,
   approval state, тип операции и физическое наличие путей внутри виртуального ПК.
...
| `repeat-mixed-search-grounding` | повторный смешанный запрос не теряет поиск, отличает найденное от недоступного и не позволяет модели выдумывать отсутствие доступа |
| `create-directory-approved` | создание папки только после подтверждения |
| `create-directory-denied-en` | английский отказ без файлового эффекта |
| `search-and-move-approved` | доверенная цепочка поиска и перемещения |
| `search-and-rename-approved` | переименование ровно одного результата |
| `search-and-trash-approved` | восстановимое удаление и `.trashinfo` |
| `create-directory-conflict` | существующий объект не перезаписывается |
| `path-traversal-refused` | path-like имя не достигает executor |
| `search-and-inspect` | R0-инспекция метаданных без подтверждения |
...
Для адресной перепроверки завершённого прогона можно подготовить отдельный
`foundation prepare --failed-from <путь-к-папке-прогона>`: в новый отчёт попадут
все ранее упавшие **типы** live-сценариев на каждом выбранном seed, включая
вариант, который в прошлый раз прошёл. Contract-тесты и model preflight также
остаются обязательными; исходный отчёт не изменяется. В `manifest.json`
сохраняются идентификатор исходного прогона и список перепроверяемых сценариев.
Это не замена последующему полному Foundation: зелёный адресный набор не
доказывает отсутствие регрессий в остальных сценариях.
...
После добавления File Operations R1 базовый профиль содержит **113 позиций**:

- обычные тесты проекта, тесты лаборатории, preflight точной `qwen3.5:2b`;
- 27 contract-сценариев и 7 дополнительных: RU/EN память на 5/35 ходах,
...
- парные end-to-end сценарии тематического и точного фразового поиска: они
  проверяют и module-owned аргументы плана, и точные пути реальной выдачи;
- каждый из 55 live-cases проходит дважды с seeds 7 и 19.

Восемь файловых сценариев можно запускать отдельным набором `focused`, не
повторяя остальную матрицу:

```powershell
python run.py run focused
```

Подготовка этого набора сама по себе не считается подтверждённым результатом:
до отдельного запуска его статус — `not_run`.

Сценарий `repeat-mixed-search-grounding` воспроизводит реальный пользовательский
диалог: поиск PDF, вопрос о нулевом результате и повторный запрос, одновременно
содержащий обычный вопрос. Он отдельно проверяет `found`, `inaccessible`, новый
вызов search capability, фактическую коррекцию в follow-up, отсутствие ложного
«у меня нет доступа» и верхний бюджет завершённого хода. Значение 90 секунд —
release-gate лаборатории, а не приемлемая UX-цель; фактические duration остаются
в trace для последующего уменьшения бюджета.
...
содержит только сгруппированные проблемы и отдельный раздел `UNKNOWN` с
безопасными rule/evidence. Автономный foundation дополнительно сохраняет те же
данные в `failures.json.unknown_details`.
...
- `found`, `inaccessible` и `copied`;
- точные `result_paths` и `copied_paths` в виртуальном namespace;
- `approval_required`;
...
Количество само по себе не доказывает правильность поиска. Search-сценарии могут
фиксировать точный набор виртуальных путей, а adaptive journey — точные файловые
эффекты и общий максимум записей. Поэтому широкий поиск с тем же количеством
объектов или копирование похожего, но неподходящего файла считаются провалом.
