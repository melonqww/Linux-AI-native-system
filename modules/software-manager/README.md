...
- snapshots синхронизируются из snapd и публикуют оставшееся время в секундах и
  днях; prepare/confirm/refresh образуют фундамент восстановления;
...
- только подтверждённые GNOME окна или процессы дают «Запущено» и действие
  `close`;
- если запуск не подтверждён за 15 секунд, возвращаются «Не удалось запустить» и
  только `retry`;
...
поддержаны только системный язык, стандартное расположение и две post-install
галочки: `pin_to_gnome` и `launch_after_install`. Неподдерживаемый язык, путь или
checkbox отклоняется до обращения к snapd.
